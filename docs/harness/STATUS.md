...
- 최근 구현: FileStore에 프로젝트 상태 전이와 변경·이벤트 이력(JSON 호환 로드 포함)을 추가했다. 후속 검토에서 `updateProject` 상태 우회 차단, 이벤트 날짜 검증, 최신순 조회, 이력 객체 복사까지 보완했다. AppStore/PgStore 계약 승격은 후속 Task 5에서 진행한다.
