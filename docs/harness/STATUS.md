...
- 최종 갱신: 2026-08-30
- 현재 위치: **공사계약 업무공간 운영 범위와 GitHub/GitLab 원격 동기화 완료, T-211 NAS 실검증 진행 대기**. 운영 API 필수 환경 전달, PostgreSQL 강제, 브라우저 API URL build-time 주입, 내부 서비스 포트 차단과 host loopback 기본 바인딩을 반영했다.
- 다음 액션: NAS Container Manager 기동 후 `docker compose config/up`, legacy `evidence_path` 0건, HTTPS 역방향 프록시와 복구 리허설을 실검증한다. 이후 파일럿 운영 흐름을 점검한다. T-201 RAG/vector API 런타임 연결은 사용자 결정에 따라 보류한다.
