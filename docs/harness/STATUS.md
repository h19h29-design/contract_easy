...
- 최종 갱신: 2026-09-12
- 최우선: **계약 문서 작성 기능(T-214)**. 공사표준계약서 1종의 입력·비공개 버전 저장/수정·미리보기·HWPX 다운로드를 `codex/contract-hwpx`에서 구현했다. 실제 한글 열기/편집/인쇄 배치는 미검증이며 NAS에는 아직 배포하지 않았다. 근거: `CONTRACT_FORMS_AUDIT.md`, `TEST_RESULTS.md`.
...
- 다음 액션: 생성 HWPX를 실제 한글에서 열기·수정·재저장하고 원본 대비 누락/인쇄 배치를 검증한다. 운영 반영·SSH 변경은 별도 승인 후 진행한다. T-201 RAG/vector API 런타임 연결은 사용자 결정으로 보류한다.
...
| 웹 UI(한국어, 마법사/검색/워크스페이스/관리자) | 기존 기능 + 공사표준계약서 시험 작성 화면 구현; 최신 검증은 TEST_RESULTS 참조 |
| HWPX 문서 작성 | PARTIAL — 1종 입력/저장/다운로드 구현, 실제 한글 검증·NAS 배포 전 |
