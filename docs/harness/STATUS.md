...
- 최종 갱신: 2026-09-12
- 최우선: **계약 문서 작성 기능(T-214/T-215)**. 공사표준계약서·착공계·준공계·대금청구서 4종의 선택·공통정보 재사용·비공개 버전 저장/수정·미리보기·HWPX 다운로드를 `codex/contract-hwpx`에서 구현했다. 실제 한글 열기/편집/인쇄 배치는 사용자 지시로 미검증 유지, NAS에는 아직 배포하지 않았다. 근거: `CONTRACT_FORMS_AUDIT.md`, `TEST_RESULTS.md`.
...
- 다음 액션: T-215 후속 3종의 코드·자동 검증 완료. 실제 한글 검증은 사용자 보류로 진행 차단 조건이 아니다. 일괄 생성·추가 붙임서식은 아직 미구현이다. 운영 반영·SSH 변경은 별도 승인 후 진행한다. T-201 RAG/vector API 런타임 연결은 사용자 결정으로 보류한다.
...
| 웹 UI(한국어, 마법사/검색/워크스페이스/관리자) | 기존 기능 + 4종 선택/공통정보 재사용 시험 작성 화면 구현; 최신 검증은 TEST_RESULTS 참조 |
| HWPX 문서 작성 | PARTIAL — 4종 입력/저장/다운로드 구현, 실제 한글 검증은 사용자 보류·NAS 배포 전 |
