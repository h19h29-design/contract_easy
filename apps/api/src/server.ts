...
import {
  FileStore, hashPassword, verifyPassword,
  STAGES, STAGE_LABELS
} from '@sen/db';
...
export async function buildApp(ctxIn?: Partial<AppContext>) {
  ...
  const cfg = getConfig();
  const store = ctxIn?.store ?? new FileStore(dirs.appStore);
  ...
  if (store.listUsers().length === 0) {
    ...
    store.ensureDefaultAdmin(hashPassword(initial));
  ...
  function loadRetriever(): HybridRetriever {
    ...
    let chunks: Chunk[] = store.getChunks();
    if (chunks.length === 0 && fs.existsSync(chunksFile)) {
      chunks = JSON.parse(fs.readFileSync(chunksFile, 'utf8')) as Chunk[];
    }
    return new HybridRetriever({ chunks, versions: store.currentVersions() });
  }
  ...
  function currentUser(req: FastifyRequest): ReturnType<FileStore['getUser']> | null {
  ...
  function requireAuth(req: FastifyRequest, reply: FastifyReply, minRole: 'USER' | 'REVIEWER' | 'ADMIN'): ReturnType<FileStore['getUser']> | null {
  ...
  app.post<{ Body: { question?: string } }>('/api/ask', async (req, reply) => {
    ...
    if (res.answered && res.answer) {
      return {
        answered: true,
        answer: res.answer,
        evidence: res.hits.map((h) => ({
          title: h.sourceTitle, url: h.sourceUrl,
          publishedAt: h.publishedAt, effectiveAt: h.effectiveAt,
          lastCheckedAt: store.getSource(h.chunk.sourceVersionId)?.versions.at(-1)?.lastCheckedAt ?? null
        })),
  ...
  app.get('/api/sources', async () => ({
    sources: store.listSources().map((s) => {
  ...
  app.post<{ Body: { question?: string; answer?: string; note?: string } }>('/api/reports', async (req, reply) => {
    ...
    const rec = store.addAnswerReport(q, req.body.answer ?? null, req.body.note ?? '');
    store.audit(null, 'report.create', 'answer_report', rec.id);
    return rec;
  });
  ...
  app.post<{ Body: { username?: string; password?: string } }>('/api/auth/login', async (req, reply) => {
    ...
    if (!user || !verifyPassword(password, user.passwordHash)) {
      store.audit(null, 'auth.login_failed', 'user', null, { username }, req.ip);
      return reply.code(401).send({ error: '아이디 또는 비밀번호가 올바르지 않습니다.' });
    }
    const token = crypto.randomUUID() + crypto.randomUUID();
    const csrfToken = crypto.randomUUID();
    store.createSession(token, user.id, csrfToken, 12 * 60 * 60 * 1000);
    reply.setCookie(SESSION_COOKIE, token, {
      httpOnly: true, sameSite: 'lax', path: '/',
      secure: process.env.NODE_ENV === 'production'
    });
    store.audit(user.id, 'auth.login', 'user', user.id, null, req.ip);
  ...
  app.get('/api/projects', async (req, reply) => {
    const user = requireAuth(req, reply, 'USER');
    if (!user) return;
    const projects = user.role === 'ADMIN' ? store.listProjects() : store.listProjects(user.id);
    return {
      projects: projects.map((p) => ({
        ...p,
        progress: computeProgress(store, p.id)
      }))
    };
  });
  ...
  app.post<{ Body: { name?: string; contractCategory?: string; estimatedPrice?: number; organizationType?: string; wizardInput?: WizardInput } }>(
    ...
    async (req, reply) => {
      ...
      store.audit(user.id, 'project.create', 'project', project.id, { name: project.name }, req.ip);
      return project;
    }
  );
  ...
  app.get<{ Params: { id: string } }>('/api/projects/:id', async (req, reply) => {
    ...
    if (!p || !store.canAccessProject(p.id, user.id, user.role)) {
      return reply.code(404).send({ error: '프로젝트가 없거나 접근 권한이 없습니다.' });
    }
    return {
      project: p,
      steps: store.stepsOf(p.id).map((s) => ({ ...s, label: STAGE_LABELS[s.stageKey] ?? s.stageKey })),
      checklist: store.checklistOf(p.id),
      progress: computeProgress(store, p.id)
    };
  });
  ...
  app.patch<{ Params: { id: string; itemId: string }, Body: { done?: boolean } }>(
    ...
    async (req, reply) => {
      ...
      const item = store.toggleChecklist(req.params.itemId, Boolean(req.body.done));
      if (!item) return reply.code(404).send({ error: '항목 없음' });
      store.audit(user.id, 'project.checklist_toggle', 'checklist_item', item.id, { done: item.done }, req.ip);
      return item;
    }
  );
  ...
  app.post<{ Params: { id: string }, Body: { changeType?: string; before?: object; after?: object; reason?: string } }>(
    ...
    async (req, reply) => {
      ...
      const id = store.addProjectChange(
        p.id,
      ...
      store.audit(user.id, 'project.change_record', 'project_change', id, { changeType: req.body.changeType }, req.ip);
      return { id };
    }
  );
  ...
  app.get('/api/admin/crawls', async (req, reply) => {
    const user = requireAuth(req, reply, 'ADMIN');
    if (!user) return;
    ...
    return {
      runs: store.listCrawlRuns(),
      sourcesCount: store.listSources().length,
      recentFailures: failures.reverse()
    };
  });

  app.get('/api/admin/rules', async (req, reply) => {
    const user = requireAuth(req, reply, 'REVIEWER');
    if (!user) return;
    return { rules: store.listRules(), conflicts: detectConflicts(store.listRules()) };
  });
  ...
  app.post<{ Params: { id: string; version: string }, Body: { action?: 'review' | 'activate' | 'reject' } }>(
    ...
    async (req, reply) => {
      ...
      let ok = false;
      if (action === 'review') { ok = Boolean(store.reviewRule(req.params.id, version, 'reviewed')); }
      else if (action === 'activate') { ok = Boolean(store.activateRule(req.params.id, version, user.displayName)); }
      else if (action === 'reject') { ok = store.rejectRule(req.params.id, version); }
      if (!ok) return reply.code(400).send({ error: '상태 변경 불가(draft는 reviewed를 거쳐야 활성화됩니다).' });
      store.audit(user.id, `rule.${action}`, 'rule', `${req.params.id}@${version}`, null, req.ip);
      return { ok: true };
    }
  );
  ...
  app.get('/api/admin/reports', async (req, reply) => {
    const user = requireAuth(req, reply, 'ADMIN');
    if (!user) return;
    return { reports: store.listAnswerReports() };
  });

  app.post<{ Params: { id: string } }>('/api/admin/reports/:id/resolve', async (req, reply) => {
    const user = requireAuth(req, reply, 'ADMIN');
    if (!user) return;
    const ok = store.resolveAnswerReport(req.params.id);
    if (!ok) return reply.code(404).send({ error: 'not found' });
    store.audit(user.id, 'report.resolve', 'answer_report', req.params.id);
    return { ok: true };
  });
...
function computeProgress(store: FileStore, projectId: string): number {
  const items = store.checklistOf(projectId);
