...
export default function ProjectDetailPage() {
  ...
  return <div className="container">
    ...
    <section className="card"><h2>상태</h2>
      <p>현재 상태: <strong>{data.project.status}</strong>{next ? ' · 다음 단계로 이동할 수 있습니다.' : ' · 마지막 단계입니다.'}</p>
        ...
        return <div key={step.id} className="project-step"><h3>{step.label} <span className={`badge ${step.status === 'done' ? 'ok' : step.status === 'in_progress' ? 'info' : ''}`}>{STATUS_LABEL[step.status]}</span></h3>
          {items.map((item) => {
            ...
            return <div key={item.id} className="check-item"><label><input type="checkbox" checked={item.done} disabled={submitting === `check:${item.id}`} onChange={(e) => void toggle(item.id, e.target.checked)} />{item.label}</label>
              {document ? <p className="source-meta">현재 증빙: <a href={`${API_URL}/api/projects/${id}/documents/${document.id}/download`}>{document.originalName}</a> · {document.mimeType} · {document.sizeBytes.toLocaleString()} bytes (교체하려면 파일 선택)</p> : <p className="source-meta">증빙 없음 — 완료 표시는 가능하지만 원문 확인이 필요합니다.</p>}
    ...
    <section className="card"><h2>변경 기록</h2>
      ...
      <ul className="history-list">{data.changes.map((change) => <li key={change.id}><strong>{change.changeType}</strong> · {change.reason} <span className="muted">({change.at.slice(0, 10)})</span></li>)}</ul>
    </section>
    ...
    <section className="card"><h2>일정</h2>
      ...
      <ul className="history-list">{data.events.map((event) => <li key={event.id}><span className="badge info">{event.displayState}</span> {event.title} · {event.dueDate}</li>)}</ul>
    </section>
