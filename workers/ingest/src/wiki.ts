...
interface DocInfo {
  key: string; // SourceVersion.menuPath[0] 또는 doc.menuPath[0]
  status: string;
}

/** 매핑: 구체적 시드 우선, 00(출처 현황)은 마지막 catch-all */
const WIKI_FILES = [
  { file: '01-계약-전체흐름.md', title: '01. 계약 전체흐름', match: (d: DocInfo) => /흐름/.test(d.key) },
  { file: '02-공사유형-구분.md', title: '02. 공사유형 구분', match: (d: DocInfo) => d.key === '계약방법 메인' || /공사유형/.test(d.key) },
  { file: '03-추정가격별-계약방법.md', title: '03. 추정가격별 계약방법', match: (_d: DocInfo) => false },
  ...
  { file: '14-첨부문서-모음.md', title: '14. 첨부문서 모음(HWP 등, 확장 파일)', match: (d: DocInfo) => /^첨부/.test(d.key) },
...
export function generateWiki(
  ...
  for (const entry of docs) {
    const info: DocInfo = {
      key: entry.version?.menuPath?.[0] ?? entry.doc.menuPath[0] ?? '',
  ...
  for (const wf of WIKI_FILES) {
    ...
    if (!entries || entries.length === 0) {
      lines.push('_아직 대응하는 수집 자료가 없습니다._');
    }
    for (const { doc, version } of entries ?? []) {
      lines.push(`## ${doc.title}`);
      lines.push('');
      lines.push(sourceMetaBlock(doc, version));
      lines.push('');
      let lastSection = '';
...
function renderTable(rows: string[][]): string {
  ...
  return [pad(rows[0]!), `|${Array(width).fill(' --- ').join('|')}|`, ...rows.slice(1).map(pad)].join('\n');
}
...
export function sourceMetaBlock(doc: NormalizedDoc, v: SourceVersion | null): string {
  ...
  return [
    ...
    `| 수집일 | ${(v?.collectedAt ?? doc.collectedAt).slice(0, 10)} |`,
    `| 마지막 확인일 | ${(v?.lastCheckedAt ?? doc.collectedAt).slice(0, 10)} |`,
    `| 적용 분야 | ${esc(doc.menuPath[0] ?? '일반')} |`,
