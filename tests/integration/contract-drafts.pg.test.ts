...
describe('PG private contract revisions', () => {
  it('serializes concurrent first saves and retains historical contents', async () => {
    ...
    const fields = { ...emptyContractFields(), workName: '원본 초안', contractAmount: '9007199254740993', claimAmount: '9007199254740881', bankAccount: '000-PG-SYNTHETIC' };
    ...
    expect((await store.getContractDraft(p.id))?.fields.claimAmount).toBe('9007199254740881');
    expect((await store.getContractDraft(p.id, 1))?.fields.bankAccount).toBe('000-PG-SYNTHETIC');
