...
## D-019 (2026-09-12) 실제 한글 검증 보류와 후속 서식
- **사용자 결정**: 현재 Mac에서 한글 검증 불가하므로 건너뛰고 다음 단계로 진행한다. 미검증을 통과로 표시하거나 한글 설치 정보를 반복 요청하지 않는다.
- **범위**: 기존 입력/비공개 version/HWPX 흐름을 재사용해 착공계·준공계·대금청구서를 추가한다. 원본은 읽기 전용으로 대조하고 매크로 실행·법적 판단값 자동 적용은 하지 않는다.
- **안전장치**: 예정일과 실제 날짜/제출일 분리, 계약/준공/기지급/청구/공제 금액 직접 입력, 은행 정보는 청구서에만 출력, 레거시 초안 이력 보존. 서버 운영·지급·배포·SSH 변경·자동 push는 포함하지 않는다.
...
## D-018 (2026-08-30) NAS 운영 배포 네트워크·설정 경계
- **결정**: 운영 API는 PostgreSQL과 정확한 `WEB_ORIGIN`을 필수로 사용한다. 웹/API host publish는 기본 loopback으로 제한하고 Synology HTTPS 역방향 프록시를 공개 경계로 둔다. PostgreSQL/Qdrant/Valkey는 host에 publish하지 않는다.
- **근거**: 개발 기본값이나 Docker 내부 DNS 주소로 운영 이미지가 기동되는 오류와, DB·내부 서비스가 NAS 네트워크에 직접 노출되는 위험을 배포 전에 차단한다.
- **운영 조건**: 공개 API URL은 웹 build-time 인자이므로 변경 시 재빌드한다. 최초 관리자 비밀번호는 관리자 생성 후 환경에서 제거한다.
